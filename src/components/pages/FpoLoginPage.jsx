...
export default function FpoLoginPage({ onLoginSuccess, onBack, onRegister }) {
  const [phone, setPhone] = useState('9822011223');
  ...
  const handleSendOtp = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.sendOtp(clean, 'fpo');
      if (res.success) {
        setOtpSent(true);
  ...
  const handleSubmit = async (e) => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.verifyOtp(phone, otp, 'fpo');
  ...
  const handleDemoLogin = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.verifyOtp('9822011223', '1234', 'fpo');
      if (res.success && res.user) {
        onLoginSuccess(res.user, 'fpo');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-amber-900/10 border border-amber-100 overflow-hidden">
          <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-8 text-white text-center relative">
            ...
            <p className="text-amber-100 text-xs">Collective Bargaining • Bulk Contracts • Live Database Sync</p>
          </div>
          ...
          <div className="p-7 space-y-5">
              ...
              <div className="flex gap-2">
                ...
                <div className="relative flex-1 flex items-center">
                  <input
                    ...
                    className="w-full pl-4 pr-10 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 font-bold text-slate-800"
                  />
            ...
            {otpSent && receivedOtpCode && (
              <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 shadow-sm animate-in fade-in space-y-2">
                ...
                <p className="text-xs text-amber-900">
                  FPO Manager Verification OTP for <strong className="font-mono">+91-{phone}</strong>:
                </p>
                <div className="flex items-center justify-between bg-white border border-amber-200 rounded-xl p-2.5">
                  <span className="font-mono text-2xl font-black tracking-widest text-amber-800">
                    {receivedOtpCode}
                  </span>
                  <button
                    type="button"
                    ...
                    className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-lg transition shadow-xs cursor-pointer flex items-center gap-1"
                  >
            ...
            {!otpSent ? (
              <button
                ...
                className="w-full py-3.5 bg-amber-100 hover:bg-amber-200 text-amber-900 font-black text-sm rounded-xl border border-amber-300 transition shadow-xs cursor-pointer disabled:opacity-50"
              >
                ...
                <div className="flex gap-2 items-center">
                  <div className="relative flex-1 flex items-center">
                    <input
                      ...
                      maxLength={4}
                      className="w-full pl-4 pr-10 py-3 rounded-xl border border-amber-300 text-base focus:outline-none focus:ring-2 focus:ring-amber-400 font-mono font-bold tracking-widest text-slate-800"
                    />
                    <VoiceInputMic onResult={val => setOtp(String(val).replace(/\D/g, '').slice(0, 4))} type="number" />
                  </div>
                  {otp.length === 4 && <CheckCircle2 className="w-6 h-6 text-amber-500 shrink-0" />}
                </div>
            ...
            >
              {loading ? '⏳ Verifying with Database...' : (<>🤝 Enter FPO Dashboard <ArrowRight className="w-4 h-4" /></>)}
            </button>

            <div className="border-t border-dashed border-slate-200 pt-4">
              <p className="text-center text-xs text-slate-400 mb-3">Quick Demo Login (Verified FPO Profile)</p>
              <button
                type="button"
              ...
              >
                🤝 Quick Demo Login (Sahyadri FPO)
              </button>
            </div>
          </div>
        </div>
        <p className="text-center text-slate-400 text-xs mt-4">🔒 SFAC-registered FPO Verification • Supabase PostgreSQL</p>
      </div>
    </div>
  );
}

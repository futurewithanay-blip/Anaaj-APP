...
import { Sprout, Phone, Lock, Eye, EyeOff, ChevronLeft, Wheat, CheckCircle2, ArrowRight, AlertCircle, Sparkles } from 'lucide-react';
...
export default function FarmerLoginPage({ onLoginSuccess, onBack, onRegister }) {
  ...
  const handleSendOtp = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.sendOtp(clean, 'farmer');
      if (res.success) {
        setOtpSent(true);
        setReceivedOtpCode(res.otp);
      } else {
        setErrorMsg(res.error || 'OTP भेजने में समस्या हुई। कृपया पुनः प्रयास करें।');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.verifyOtp(phone, otp, 'farmer');
      if (res.success && res.user) {
        onLoginSuccess(res.user, 'farmer');
      } else {
        setErrorMsg(res.error || 'अमान्य OTP कोड! कृपया दोबारा जाँचें।');
      }
    } catch (err) {
      setErrorMsg('सत्यापन के दौरान त्रुटि हुई।');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDemoLogin = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await authService.verifyOtp('9876543210', '1234', 'farmer');
      if (res.success && res.user) {
        onLoginSuccess(res.user, 'farmer');
      }
    } catch (err) {
      setErrorMsg('डेमो लॉगिन में समस्या हुई।');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-white flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-emerald-900/10 border border-emerald-100 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-emerald-700 to-green-600 p-8 text-white text-center relative overflow-hidden">
            ...
            <p className="text-emerald-100 text-xs">Direct Selling • MSP Benchmark • Live Supabase Linkage</p>
          </div>
          ...
          <div className="p-7 space-y-5">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-3 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 font-bold text-sm rounded-xl shadow-xs transition cursor-pointer"
            >
              ...
              <div className="flex gap-2">
                ...
                <div className="relative flex-1 flex items-center">
                  <input
                    ...
                    className="w-full pl-4 pr-10 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 font-bold text-slate-800"
                  />
            ...
            {otpSent && receivedOtpCode && (
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 shadow-sm animate-in fade-in space-y-2">
                ...
                <p className="text-xs text-emerald-900">
                  Verification OTP for <strong className="font-mono">+91-{phone}</strong>:
                </p>
                <div className="flex items-center justify-between bg-white border border-emerald-200 rounded-xl p-2.5">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition shadow-xs cursor-pointer flex items-center gap-1"
                  >
            ...
            {!otpSent ? (
              <button
                ...
                className="w-full py-3.5 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-black text-sm rounded-xl border border-emerald-300 transition shadow-xs cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading ? '⏳ Generating Secure OTP...' : '📲 Send OTP / ओटीपी भेजें'}
              </button>
            ) : (
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1.5">Enter 4-Digit OTP / ओटीपी कोड *</label>
                <div className="flex gap-2 items-center">
                  <div className="relative flex-1 flex items-center">
                    <input
                      ...
                      maxLength={4}
                      className="w-full pl-4 pr-10 py-3 rounded-xl border border-emerald-300 text-base focus:outline-none focus:ring-2 focus:ring-emerald-400 font-mono font-bold tracking-widest text-slate-800"
                    />
                    <VoiceInputMic onResult={val => setOtp(String(val).replace(/\D/g, '').slice(0, 4))} type="number" />
                  </div>
                  {otp.length === 4 && <CheckCircle2 className="w-6 h-6 text-emerald-500 shrink-0" />}
                </div>
                <div className="flex justify-between items-center mt-1.5">
                  <p className="text-[11px] text-slate-500">Didn't receive? या नया कोड चाहिए?</p>
                  <button
                    type="button"
                    onClick={handleSendOtp}
            ...
            >
              {loading ? '⏳ Verifying with Database...' : (
                <>
                  🌾 Enter Farmer Dashboard / किसान डैशबोर्ड
            ...
            <div className="border-t border-dashed border-slate-200 pt-4">
              <p className="text-center text-xs text-slate-400 mb-3">Quick Demo Login (Verified Farmer Profile)</p>
              <button
                type="button"
                onClick={handleDemoLogin}
                className="w-full py-3 bg-amber-50 border border-amber-200 text-amber-800 font-bold text-sm rounded-xl hover:bg-amber-100 transition flex items-center justify-center gap-2 cursor-pointer"
              >
                👨‍🌾 Quick Demo Login (Dnyaneshwar Patil)
              </button>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-slate-400 text-xs mt-4">
          🔒 Secured by Supabase Cloud PostgreSQL • Aadhaar & Phone Linked
        </p>
      </div>
    </div>
  );
